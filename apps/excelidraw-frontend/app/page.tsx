...
// Import types from React and other libraries
import type { FC } from 'react';
import type { IconType } from 'react-icons';

import { Button } from "@repo/ui/button";
// The Card component is no longer needed as we replace it with a div
...
const HeroSection: FC<HeroSectionProps> = ({ isLoggedIn, handleLogout }) => (
  <header className="relative z-10 overflow-hidden py-24">
    ...
    <div className="relative container mx-auto px-4 text-center">
      ...
      <div className="mt-12 flex items-center justify-center gap-x-6 animate-fade-in-up [animation-delay:600ms]">
        {isLoggedIn ? (
          <Button onClick={handleLogout} variant="ghost" size="lg" className="h-14 px-10 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white shadow-2xl transform hover:scale-110 hover:-translate-y-1 transition-all duration-300 border border-red-500/50 hover:border-red-400 hover:shadow-red-500/25">
          ...
          <>
            <Link href="/signin">
              <Button variant="ghost" size="lg" className="h-14 px-10 border-2 border-gray-600 text-gray-300 hover:bg-white hover:text-black hover:border-white transition-all duration-300 transform hover:scale-110 hover:-translate-y-1 shadow-xl hover:shadow-2xl group">
...
// ▼▼▼ THIS COMPONENT HAS BEEN FIXED ▼▼▼
const FeatureCard: FC<Feature> = ({ icon: Icon, title, desc, color, hoverColor }) => (
  // FIX: Replaced <Card> with <div> to prevent the parent from being a <p> tag.
          ...
          <div className="mt-12 flex items-center justify-center gap-x-8 flex-wrap">
            {isLoggedIn ? (
              <>
                ...
                <Link href="/slug">
                  <Button variant="outline" size="lg" className="h-16 px-12 border-2 border-purple-500 text-purple-400 hover:bg-purple-500 hover:text-white hover:border-purple-400 transition-all duration-300 transform hover:scale-110 hover:-translate-y-2 shadow-2xl hover:shadow-purple-500/25 group">
      ...
      <div className="container mx-auto px-4 py-16 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          <div className="col-span-1 md:col-span-2">
            <h3 className="text-2xl font-bold text-white mb-4 hover:text-purple-300 transition-colors duration-300">ValhallaBrush</h3>
            <p className="text-gray-400 max-w-md leading-relaxed">The ultimate collaborative drawing platform that brings teams together through the power of visual creativity and real-time collaboration.</p>
          </div>
          {Object.entries(FOOTER_LINKS).map(([title, links]) => (
            <div key={title}>
              ...
              <ul className="space-y-2">
                {links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-gray-400 hover:text-purple-400 transition-colors duration-300 hover:translate-x-1 inline-block">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="flex flex-col items-center justify-between gap-6 sm:flex-row pt-8 border-t border-gray-800">
          ...
          <div className="flex space-x-6">
            <a href="https://github.com" className="text-gray-500 hover:text-white hover:scale-125 transition-all duration-300 transform"><FiGithub className="h-6 w-6" /></a>
            <a href="#" className="text-gray-500 hover:text-white hover:scale-125 transition-all duration-300 transform"><FiDownload className="h-6 w-6" /></a>
            <a href="#" className="text-gray-500 hover:text-white hover:scale-125 transition-all duration-300 transform"><FiShare2 className="h-6 w-6" /></a>
          </div>
        </div>
      </div>
    </footer>
